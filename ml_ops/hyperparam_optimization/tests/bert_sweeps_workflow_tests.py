...
from ml_ops.hyperparam_optimization.models import (
    BertEvalRequest,
    BertEvalResult,
    BertFineTuneConfig,
    BertFineTuneRequest,
...
# Shared mock activities used by all tests.
...
class TestBertSweepsWorkflows:
    ...
    async def test_ladder_sweep_workflow_with_mocked_activities(
        ...
        # The CoordinatorWorkflow dispatches training children to
        # "bert-training-task-queue", so we need a worker on that queue too.
        async with (
            Worker(
                client,
            ...
            # Note: we check type().__name__ rather than isinstance() because
            # the Temporal sandbox imports models into a separate namespace.
            assert isinstance(results, list)
            assert results
            for item in results:
                assert type(item).__name__ == "BertEvalResult"
